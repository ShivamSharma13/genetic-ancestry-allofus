...
storageDirectory = '/home/jupyter/workspaces/geneticancestryinference/Data/v7-Genotype/'

combineGenotypes = function(chromosome = NULL, runningSet = NULL, addInSet = NULL, fillAddIn = FALSE) {

    new = paste0(runningSet, addInSet)
    outputDirectory = paste0(storageDirectory, '1KGPHGDPAOU', '/')
...
mergeChromosomes = function(runningSet, addInSet, fstFilter = FALSE, fillAddIn = FALSE) { 
    ...
    #outputPrefix = paste0(storageDirectory, 'unphased/', paste0(runningSet, addInSet), '/extractedChr')
    outputPrefix = paste0(storageDirectory, '1KGPHGDPAOU', '/extractedChr')    
    ...
    #command = paste('if [ -f', misSNPFile, ']; then rm', misSNPFile, ';fi')
    command = paste('plink',
        '--keep-allele-order',
    ...
    if (!is.na(file.info(misSNPFile)[1,'size'])) {
        ...
        #filteredPrefix = paste0(storageDirectory, 'unphased/', runningSet, addInSet, '/filteredExtractedChr')
        filteredPrefix = paste0(storageDirectory, '1KGPHGDPAOU', '/filteredExtractedChr')
    ...
    #noIndelPrefix = paste0(storageDirectory, 'unphased/', runningSet, addInSet, '/noIndelExtractedChrAll')
    noIndelPrefix = paste0(storageDirectory, '1KGPHGDPAOU', '/noIndelExtractedChrAll')
    ...
    ## Fst prune this thing
    if (fstFilter) {

        ## First, find the addin populations
        #addInFAMFile = paste0(storageDirectory, 'unphased/', paste0(runningSet, addInSet), '/uniqueAddInChr1.fam')
        addInFAMFile = paste0(storageDirectory, '1KGPHGDPAOU', '/uniqueAddInChr1.fam')
        ...
        if (nrow(highFst) > 0) {
            ...
            #fstPrunedPrefix = paste0(storageDirectory, 'unphased/', paste0(runningSet, addInSet), '/fstPrunedChrAll')
            fstPrunedPrefix = paste0(storageDirectory, '1KGPHGDPAOU', '/fstPrunedChrAll')
    ...
    ##############################################
    ###Andy takes off to HWE pruning from here.###
    ##############################################
             
    ## And prune for funsies
    #prunedPrefix = paste0(storageDirectory, 'unphased/', paste0(runningSet, addInSet), '/extractedChrAllPruned')
    prunedPrefix = paste0(storageDirectory, '1KGPHGDPAOU', '/extractedChrAllPruned')
...
mergeChromosomes(runningSet='1KGPHGDP', addInSet='AllofUs', fstFilter=TRUE)
