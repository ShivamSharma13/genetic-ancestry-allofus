...
#Read the EHR cohort file.
disease_cohort = as.data.frame(fread("Data/Metadata/AllDiseases.AllStandardSNOMED.tsv"))
dim(disease_cohort)
head(disease_cohort)
...
count(disease_cohort, source_vocabulary)
...
#Get the date of assesment to implement the rule of two if required..
...
output_directory = "Data/PhecodeFiles/Cohorts/"
...
createPheCodeFile = function(phecode_map_row_index){
    ...
    #Take the controls which are carrying an exclusion criteria phecode, and mark them as "Excluded"
...
mclapply(phecode_map_aggregated_indices, function(i) createPheCodeFile(i), mc.cores = 4)
